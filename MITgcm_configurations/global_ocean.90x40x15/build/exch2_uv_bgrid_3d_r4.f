...
CBOP
C     !ROUTINE: EXCH2_UV_BGRID_3D_R4

C     !INTERFACE:
      SUBROUTINE EXCH2_UV_BGRID_3D_R4(
     U                                 uPhi, vPhi,
     I                                 withSigns, myNz, myThid )

C     !DESCRIPTION:
C*=====================================================================*
C  Purpose: SUBROUTINE EXCH2_UV_BGRID_3D_R4
...
C     msgBuf        :: Informational/error message buffer
      ...
      Real*4 uPhiNW(local_maxDim,nSx,nSy), uPhiSE(local_maxDim,nSx,nSy)
      Real*4 vPhiNW(local_maxDim,nSx,nSy), vPhiSE(local_maxDim,nSx,nSy)
      Real*4 uLoc(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*4 vLoc(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*4 negOne
...
C---  using CubedSphereExchange:
       IF ( myNz.GT.local_maxDim ) THEN
         WRITE(msgBuf,'(2A,2(I4,A))') 'EXCH2_UV_BGRID_3D_R4 :',
     &     ' 3rd dimension=', myNz,
     &     ' exceeds local_maxDim (=', local_maxDim, ' )'
         CALL PRINT_ERROR( msgBuf , myThid )
         WRITE(msgBuf,'(2A)') 'EXCH2_UV_BGRID_3D_R4 :',
     &     ' Increase "local_maxDim" and recompile'
         CALL PRINT_ERROR( msgBuf , myThid )
         STOP 'ABNORMAL END: S/R EXCH2_UV_BGRID_3D_R4'
       ENDIF
