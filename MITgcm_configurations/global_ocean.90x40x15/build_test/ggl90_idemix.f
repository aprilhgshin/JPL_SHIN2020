...
C     === Local variables ===
      ...
      Real*8  deltaTggl90
      Real*8  fxa,fxb,fxc,cstar
      Real*8  dfx(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8  dfy(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8  IDEMIX_gofx2,IDEMIX_hofx1
      Real*8  delta(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  bN0(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      ...
      Real*8  osborn_diff(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  c0(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  forc(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  gm_forc(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
CEOP
...
c-----------------------------------------------------------------------
     ...
     &                  * sigmaR(i,j,k)
         fxb = max( 1.D-6, abs( fCori(i,j,bi,bj) ))
...
c-----------------------------------------------------------------------
     ...
     &             cstar*IDEMIX_gamma*IDEMIX_gofx2(fxa))
          IDEMIX_V0(i,j,k,bi,bj)=max(0.D0,
...
C     tau_h V0**2 *dt/dx**2 < 0.25 <=> V0 < sqrt( 0.25 * dx**2/(dt*tau_h) )
       fxa = sqrt( 1.D0/( deltaTggl90 * IDEMIX_tau_h ) )
       DO k=2,Nr
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          fxb = 0.5*min( dxF(i,j,bi,bj), dyF(i,j,bi,bj) )*fxa
...
c     vertically integrated forcing
     ...
     &              +max( 0.D0,Kwz(i,j,k,bi,bj)*Nsquare(i,j,k) )
     &               *drC(k)*hFacI(i,j,k)
         ENDDO
        ENDDO
       ENDDO
...
c-----------------------------------------------------------------------
       DO k=2,Nr
        DO j=1-OLy,sNy+OLy
         ...
         DO i=1-OLx+1,sNx+OLx
          fxa = IDEMIX_tau_h*0.5D0*(
     &        IDEMIX_V0(i-1,j,k,bi,bj)*maskC(i-1,j,k,bi,bj)
     &       +IDEMIX_V0(i  ,j,k,bi,bj)*maskC(i  ,j,k,bi,bj))
          dfx(i,j) = -fxa*dyG(i,j,bi,bj)*drC(k)
     &                *(min(.5D0,hFacW(i,j,k-1,bi,bj) ) +
     &                  min(.5D0,hFacW(i,j,k  ,bi,bj) ) )
     &      *recip_dxC(i,j,bi,bj)
     ...
     &       +IDEMIX_V0(i,j-1,k,bi,bj)*maskC(i,j-1,k,bi,bj) )
          dfy(i,j) = -fxa*dxG(i,j,bi,bj)*drC(k)
     &                *(min(.5D0,hFacS(i,j,k-1,bi,bj) ) +
     &                  min(.5D0,hFacS(i,j,k  ,bi,bj) ) )
     &      *recip_dyC(i,j,bi,bj)
     &      *(IDEMIX_V0(i,j  ,k,bi,bj)*IDEMIX_E(i,j  ,k,bi,bj)
...
C--   Center diagonal
      ...
      DO k=2,Nr
       ...
       DO j=jMin,jMax
        DO i=iMin,iMax
          b3d(i,j,k) = 1.D0 + deltaTggl90*IDEMIX_tau_d(i,j,k,bi,bj)
...
c-----------------------------------------------------------------------
     ...
     &         *IDEMIX_E(i,j,k,bi,bj)*IDEMIX_E(i,j,k,bi,bj)
     &     /max(1.D-12,Nsquare(i,j,k))*maskC(i,j,k,bi,bj)
...
C     helper functions
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
      Real*8 FUNCTION IDEMIX_gofx2(xx)
      IMPLICIT NONE
      Real*8 x,c,xx
      Real*8 pi
      ...
      END
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
      Real*8 FUNCTION IDEMIX_hofx1(x)
      IMPLICIT NONE
      Real*8 x
      Real*8 pi
      PARAMETER( pi = 3.14159265358979323846264338327950588d0 )
