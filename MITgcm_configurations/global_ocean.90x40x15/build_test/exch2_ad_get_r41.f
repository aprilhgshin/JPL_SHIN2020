...
CBOP 0
C !ROUTINE: EXCH2_AD_GET_R41

C !INTERFACE:
      SUBROUTINE EXCH2_AD_GET_R41(
...
C     myThid        :: my Thread Id. number
      ...
      Real*4     array(i1Lo:i1Hi,j1Lo:j1Hi,k1Lo:k1Hi)
...
C      this source tile Id "soT" (note: this is saved in W2_tileIndex array)
      ...
      ELSEIF ( commSetting .EQ. 'M' ) THEN
       ...
       sNb = nN
      ELSE
       STOP 'EXCH2_AD_GET_R41:: commSetting VALUE IS INVALID'
      ENDIF
...
C     Read from e2Bufr1_R4(iBufr,sNb,sBi,sBj,sLv)
         ...
         e2Bufr1_R4(iLoc,sNb,sBi,sBj,sLv) = array(itl,jtl,ktl)
         array(itl,jtl,ktl) =  0.
        ENDDO
       ENDDO
      ENDDO
      IF ( iBufr .GT. e2BufrRecSize ) THEN
        WRITE(msgBuf,'(2A,I9,A,I9)') 'EXCH2_AD_GET_R41:',
     &   ' iBufr =', iBufr, ' exceeds E2BUFR size=', e2BufrRecSize
        CALL PRINT_ERROR ( msgBuf, myThid )
        STOP 'ABNORMAL END: S/R EXCH2_AD_GET_R41 (iBufr over limit)'
      ENDIF
