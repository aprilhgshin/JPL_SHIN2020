...
CEOP
      ...
      Real*8    GGL90ck, GGL90ceps
      Real*8    GGL90alpha, GGL90m2
      Real*8    GGL90diffTKEh
      Real*8    GGL90mixingLengthMin
      Real*8    GGL90TKEmin, GGL90TKEsurfMin, GGL90TKEbottom
      Real*8    GGL90viscMax, GGL90diffMax
      Real*8    GGL90dumpFreq, GGL90taveFreq
      INTEGER mxlMaxFlag
     ...
     &     GGL90dumpFreq, GGL90taveFreq, mxlMaxFlag
      ...
      Real*8 GGL90TKE    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 GGL90viscArU(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 GGL90viscArV(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 GGL90diffKr (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
...
c   IDEMIX parameter
c-----------------------------------------------------------------------
      Real*8  IDEMIX_tau_v,IDEMIX_tau_h,IDEMIX_gamma, IDEMIX_jstar
      Real*8  IDEMIX_mu0, IDEMIX_diff_min
      Real*8  IDEMIX_mixing_efficiency, IDEMIX_diff_max
      Real*8  IDEMIX_frac_F_b, IDEMIX_frac_F_s
c-----------------------------------------------------------------------
c      IDEMIX 3-d fields
c-----------------------------------------------------------------------
      Real*8 IDEMIX_E    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 IDEMIX_V0   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 IDEMIX_tau_d(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
c-----------------------------------------------------------------------
c      IDEMIX 2-d fields
c-----------------------------------------------------------------------
      Real*8 IDEMIX_F_B(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 IDEMIX_F_S(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C     TKEPrandtlNumber :: here, an empirical function of the Richardson number
      ...
      Real*8     explDissFac, implDissFac
      Real*8     uStarSquare
      Real*8     verticalShear(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     KappaM(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     KappaH
c     Real*8     Nsquare
      Real*8     Nsquare(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8     deltaTggl90
c     Real*8     SQRTTKE
      Real*8     SQRTTKE(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8     RiNumber
      Real*8     IDEMIX_RiNumber
      Real*8     TKEdissipation
      Real*8     tempU, tempUp, tempV, tempVp, prTemp
      Real*8     MaxLength, tmpmlx, tmpVisc
      Real*8     TKEPrandtlNumber (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8     GGL90mixingLength(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8     rMixingLength    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8     mxLength_Dn      (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8     KappaE           (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8     totalDepth       (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     GGL90visctmp     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8     surf_flx_tke     (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
C     hFac(I)  :: fractional thickness of W-cell
      Real*8       hFac
      Real*8       hFacI(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8 recip_hFacI(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
...
C     case of using IDEMIX.
       DO K=1,Nr
        ...
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          hFac =
     &         MIN(.5D0,hFacC(i,j,km1,bi,bj) ) +
     &         MIN(.5D0,hFacC(i,j,k  ,bi,bj) )
          recip_hFacI(I,J,K)=0.D0
          IF ( hFac .NE. 0.D0 )
     &         recip_hFacI(I,J,K)=1.D0/hFac
          hFacI(i,j,k) = hFac
         ENDDO
        ENDDO
       ENDDO
...
C     Initialize local fields
      DO k = 1, Nr
       DO j=1-OLy,sNy+OLy
        DO i=1-OLx,sNx+OLx
         rMixingLength(i,j,k)     = 0.D0
         mxLength_Dn(i,j,k)       = 0.D0
         GGL90visctmp(i,j,k)      = 0.D0
         KappaE(i,j,k)            = 0.D0
         TKEPrandtlNumber(i,j,k)  = 1.D0
...
C--   Impose minimum mixing length to avoid division by zero
     ...
     &                                   GGL90mixingLengthMin)
          rMixingLength(i,j,k) = 1.D0 / GGL90mixingLength(i,j,k)
         ENDDO
        ENDDO
       ENDDO
...
c         MaxLength=MAX(MaxLength,20.D0)
          GGL90mixingLength(i,j,k) = MIN(GGL90mixingLength(i,j,k),
     &                                   MaxLength)
         ENDDO
        ENDDO
     ...
     &                                  mxLength_Dn(i,j,k))
          ...
          rMixingLength(i,j,k) = 1.D0 / tmpmlx
         ENDDO
        ENDDO
       ENDDO
...
C     account for partical cell factor in vertical shear:
     ...
     &                       * recip_hFacI(i,j,k)*recip_hFacI(i,j,k)
          RiNumber = MAX(Nsquare(i,j,k),0.D0)
     &         /(verticalShear(i,j)+GGL90eps)
CML         IDEMIX_RiNumber = 1./GGL90eps
          IDEMIX_RiNumber = MAX( KappaM(i,j)*Nsquare(i,j,k), 0.D0)/
     &     (GGL90eps+IDEMIX_tau_d(i,j,k,bi,bj)*IDEMIX_E(i,j,k,bi,bj)**2)
          prTemp         = MIN(5.*RiNumber, 6.6D0*IDEMIX_RiNumber)
          TKEPrandtlNumber(i,j,k) = MIN(10.D0,prTemp)
          TKEPrandtlNumber(i,j,k) = MAX( 1.D0,TKEPrandtlNumber(i,j,k))
         ENDDO
        ENDDO
       ELSE
        DO j=jMin,jMax
     ...
     &         /(verticalShear(i,j)+GGL90eps)
          prTemp = 1.D0
          IF ( RiNumber .GE. 0.2D0 ) prTemp = 5.D0 * RiNumber
          TKEPrandtlNumber(i,j,k) = MIN(10.D0,prTemp)
         ENDDO
        ENDDO
       ENDIF
...
C-    No need for maskC(k-1) with recip_hFacC(k-1)
     ...
     &        *.5D0*(KappaE(i,j, k )+KappaE(i,j,km1))
...
C-    No need for maskC(k) with recip_hFacC(k)
     ...
     &        *.5D0*(KappaE(i,j,k)+KappaE(i,j,kp1))
...
C--   Center diagonal
      DO k=1,Nr
       ...
       DO j=jMin,jMax
        DO i=iMin,iMax
         b3d(i,j,k) = 1.D0 - c3d(i,j,k) - a3d(i,j,k)
...
C     estimate friction velocity uStar from surface forcing
        uStarSquare = SQRT(
     &    ( .5D0*( surfaceForcingU(i,  j,  bi,bj)
     &              + surfaceForcingU(i+1,j,  bi,bj) ) )**2
     &  + ( .5D0*( surfaceForcingV(i,  j,  bi,bj)
...
C     ===============================
      ...
      DO k=2,Nr
       DO j=1,sNy
        DO i=1,sNx+1
         tmpVisc = maskW(i,j,k,bi,bj) * halfRL
     ...
     &           + GGL90visctmp(i,j,k) )
      ...
      DO k=2,Nr
       DO j=1,sNy+1
        DO i=1,sNx
         tmpVisc = maskS(i,j,k,bi,bj) * halfRL
