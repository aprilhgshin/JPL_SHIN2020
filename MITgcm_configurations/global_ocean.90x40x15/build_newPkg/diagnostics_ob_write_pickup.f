...
C     DIAGOB 3-dim. fields
      Real*8 diagOB_StatScal1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 diagOB_StatScal2(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 diagOB_StatVelU(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 diagOB_StatVelV(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      COMMON /diagOB_STATE_3D/
     &    diagOB_StatScal1, diagOB_StatScal2,
     &    diagOB_StatVelU,  diagOB_StatVelV
C     DIAGOB 2-dim. fields
      Real*8 diagOB_Surf1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 diagOB_Surf2(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      COMMON /diagOB_STATE_2D/
     &    diagOB_Surf1, diagOB_Surf2

      Real*8 diagOB_TendScal1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 diagOB_TendScal2(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 diagOB_TendVelU(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 diagOB_TendVelV(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
...
C     record number < 0 : a hack not to write meta files now:
     ...
     &                        diagOB_StatScal1, -j, myIter, myThid )
        IF (j.LE.listDim) wrFldList(j) = 'diagOBSta1'

        j = j + 1
        CALL WRITE_REC_3D_RL( fn, fp, Nr,
     &                        diagOB_StatScal2, -j, myIter, myThid )
        IF (j.LE.listDim) wrFldList(j) = 'diagOBSta2'

        j = j + 1
        CALL WRITE_REC_3D_RL( fn, fp, Nr,
     &                        diagOB_StatVelU, -j, myIter, myThid )
        IF (j.LE.listDim) wrFldList(j) = 'diagOBStaU'
        j = j + 1
        CALL WRITE_REC_3D_RL( fn, fp, Nr,
     &                        diagOB_StatVelV, -j, myIter, myThid )
        IF (j.LE.listDim) wrFldList(j) = 'diagOBStaV'
...
C-    switch to 2-D fields:
     ...
     &                        diagOB_Surf1, nj, myIter, myThid )
        IF (j.LE.listDim) wrFldList(j) = 'diagOBSur1'

        j = j + 1
        nj = nj-1
        CALL WRITE_REC_3D_RL( fn, fp, 1,
     &                        diagOB_Surf2, nj, myIter, myThid )
        IF (j.LE.listDim) wrFldList(j) = 'diagOBSur2'
