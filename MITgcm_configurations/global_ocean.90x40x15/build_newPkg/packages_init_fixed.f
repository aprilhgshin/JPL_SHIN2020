...
C--   Old place where CTRL_INIT & CTRL_ADMTLM used to be called
      ...
      IF (useDiagnostics_ob) THEN
       IF (debugMode) CALL DEBUG_CALL('DIAGNOSTICS_OB_INIT_FIXED',
     &                                 myThid)
       CALL DIAGNOSTICS_OB_INIT_FIXED(myThid)
      ENDIF
...
C# ifdef 
C        IF (debugMode) CALL DEBUG_CALL('DIAGNOSTICS_OB_INIT_FIXED',
C     &                                  myThid)
C# endif
C        CALL DIAGNOSTICS_OB_INIT_FIXED( myThid )
C      ENDIF
C#endif
