...
C-    additional parameters:
      ...
      Real*8 diagOB_param1
      Real*8 diagOB_param2
...
C     These common block variables are initialized in diagnostics_ob_init_varia.F
      ...
      Real*8 ob_subMask(nOB_mask,1-Olx:sNx+Olx,1-Oly:sNy+Oly,nSx,nSy)
      Real*8 subField_avg(nOB_fld,1-Olx:sNx+Olx,1-Oly:sNy+Oly,nSx,nSy)
      Real*8 subField(nOB_fld,1-Olx:sNx+Olx,1-Oly:sNy+Oly,nSx,nSy)


      Real*8 subFieldOnMask(nOB_fld, sNx + sNy + 1)
      Real*8 sub_global_indices(sNx + sNy + 1)
      INTEGER lookup_table(nOB_mask, Ny*Nx)
      Real*8 global_ob((sNy+sNx)*(nPx*nPy))

      Real*8 avgPeriod_ob
      INTEGER deltaT_ob
      INTEGER totPhase_ob
      Real*8 nTimeSteps_ob
      Real*8 time_passed
C     ==========================================================================
...
C      Although we read REAL*4 values, most built in functions have Real*8 input which by default is Real*8.
C      So global_mask, ob_subMask, and bath_subMask are set as Real*8 or Real*8.
       Real*8 global_mask(Nx,Ny,nSx,nSy)
       Real*8 recv_arr(sNx + sNy + 1)
       Real*8 g_indices(nPx*nPy, sNx + sNy + 1)
C       Real*8 timeave_cum(Nr, nSx, nSy)
...
C     == Routine arguments Function DIFFERENT_MULTIPLE ==
...
C          DO j=1,nSx
C            DO k=1,nSy
C              timeave_cum(Nr, nSx, nSy) = deltaT_ob
C            ENDDO
C          ENDDO
C        ENDDO
...
C        Reading from binary file to retrieve open boundary sub mask
         DO i=1, nOB_mask
           print *, "ob_filename:", ob_filenames(i)
           CALL READ_REC_XY_RL( ob_filenames(i),
     &               ob_subMask(i,:,:,:,:),1, 0, myThid )
           CALL EXCH_XY_RL ( ob_subMask(i,:,:,:,:),  myThid )
C              Plotting for debugging
     ...
     &                        'Open Boundary Submask', -1, myThid )
         ...
         DO i=1, nOB_fld
           print *, "ob_fldNames", ob_fldNames(i)
         ENDDO
...
C          ob_fildFiles(1) = 'bathymetry.bin'
C          ob_fildFiles(2) = 'lev_t.bin'
C          ob_fldNames(1) = 'bath'
C          ob_fldNames(2) = 'theta'

C            CALL WRITE_SUB_BIN(mpiMyId, mpiPx, mpiPy, ob_subMask,
C     &                          "mask", ob_id)
        print *, "called SET_SUBFIELDS"
...
C          Compute time average of all sub fields
         CALL TIMEAVE_OB_FLD(myThid)
C         print *, "ob_output: different multiple"
C         CALL PRINT_FLOAT_ARR(subField_avg(1,:,:,:,:),ob_subMask_size)
...
C          Computing global indices, storing values on open boundary points
     ...
     &                              j*(iG-1)+i
                  ...
                  if (ob_subMask(ob_id,i,j,bi,bj) .ge. 1.0 ) then
                    if (print_debug .eq. 1) then
                     print *,"OB_EXTRACT_OUTPUT: ob_subMask gt 1:",
     &                    ob_subMask(ob_id,i,j,bi,bj)
                     print *,"OB_EXTRACT_OUTPUT: i j of ob_subMask",
     &                    " gt 1", i, j
                    end if
...
C                                   if number of valid elements in arrays > 0:
     ...
     &                 recv_arr,global_ob, lookup_table, ob_id)
     ...
     &     "for tag", fld+1, "with total", total_valid, "valid elements"
                  ...
                  call WRITE_GLOBAL_BIN(global_ob,total_valid,
     &                          ob_fldNames(fld), ob_id, mpiPx, mpiPy)
...
CEH3 ;;; End: ***
     ...
     &           subField_avg(fld,k,j,bi,bj)+subField(fld,k,j,bi,bj)
      ...
      SUBROUTINE TIMEAVE_OB_FLD(myThid)
C       Cumulate Diagnostics_ob fields and number of time steps (for Time Average)
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C          CALL WRITE_SUB_BIN(mpiMyId, mpiPx, mpiPy,
C     &           subField_avg(fld,:,:,:,:), ob_fldNames(fld), ob_id)
        ENDDO
     ...
     &                           lookup_table, ob_id)
         ...
         Real*8 global_indices(sNx + sNy + 1)
         Real*8 ob_values(sNx + sNy + 1)
         Real*8 global_ob((sNy+sNx)*(nPx*nPy))
         INTEGER lookup_table(nOB_mask, Ny*Nx)
...
C      Writing all sub arrays read from binary fie containing global array
       ...
       character*2 dir
       character*30 filename
       Real*8 subMask(1-Olx:sNx+Olx,1-Oly:sNy+Oly,nSx,nSy)
...
C       Real*8 fld(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
       CHARACTER*(*) fldNam
       INTEGER myThid
