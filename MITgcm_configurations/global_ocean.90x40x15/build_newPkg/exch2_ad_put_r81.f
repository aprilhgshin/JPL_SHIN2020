...
CBOP 0
C !ROUTINE: EXCH2_AD_PUT_R81

C !INTERFACE:
      SUBROUTINE EXCH2_AD_PUT_R81 (
      ...
      DO ktl=tKlo,tKhi,tKStride
       DO jtl=tJLo, tJHi, tjStride
        DO itl=tILo, tIHi, tiStride
         ...
         array(isl,jsl,ktl) = array(isl,jsl,ktl) + e2Bufr1_R8(iLoc)
         e2Bufr1_R8(iLoc) = 0.D0
        ENDDO
       ENDDO
      ENDDO
      IF ( iBufr .GT. e2BufrRecSize ) THEN
C     Ran off end of buffer. This should not happen
        WRITE(msgBuf,'(2A,I9,A,I9)') 'EXCH2_AD_PUT_R81:',
     &   ' iBufr =', iBufr, ' exceeds E2BUFR size=', e2BufrRecSize
        CALL PRINT_ERROR ( msgBuf, myThid )
        STOP 'ABNORMAL END: S/R EXCH2_AD_PUT_R81 (iBufr over limit)'
      ENDIF
